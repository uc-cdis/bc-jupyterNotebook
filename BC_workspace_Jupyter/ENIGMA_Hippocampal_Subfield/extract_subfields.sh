...
printf "%g," `grep -w ${x} ${subj_id}/lh.hippoSfVolumes-T1.v10.txt | awk '{print $2}'` >> $outputF
done

for x in Hippocampal_tail subiculum CA1 hippocampal-fissure presubiculum parasubiculum molecular_layer_HP GC-ML-DG CA3 CA4 fimbria HATA Whole_hippocampus; do
printf "%g," `grep -w ${x} ${subj_id}/rh.hippoSfVolumes-T1.v10.txt | awk '{print $2}'` >> $outputF
done


for x in Left-Hippocampus Right-Hippocampus; do
printf "%g," `grep  ${x} ${subj_id}/aseg.stats | awk '{print $4}'` >> $outputF
done

printf "%g," `cat ${subj_id}/aseg.stats | grep IntraCranialVol | awk -F, '{print $4}'` >> $outputF
printf "%g," `cat ${subj_id}/aseg.stats | grep 'Brain Segmentation Volume,' | awk -F, '{print $4}'` >> $outputF
printf "%g" `cat ${subj_id}/aseg.stats | grep  'Total gray matter volume' | awk -F, '{print $4}'` >> $outputF
